...
#define MIN_PUNTOS 3000
#define MAX_PUNTOS 4000
#define CANT_APROX 5
#define MAX_HILOS 25
...
void fechaActual()
{
    ...
    char tiempo_act[100];
    strftime(tiempo_act, sizeof(tiempo_act), "%Y-%m-%d %H:%M:%S", local_time);
    printf("----------------------------------------->La Fecha y Hora local son: %s\n", tiempo_act);
}
...
void escribir_archivo(char *nombre_archivo, char *modo)
{
    ...
    fprintf(archivo,"----------------------------------------->La Fecha y Hora local son: %s\n", tiempo_act);
    fclose(archivo);
}
...
int main(int argc, char *argv[])
{
    ...
    if (cant_hilos < 15 || cant_hilos > 25)
    {
    ...
    fechaActual();//------------------------------------>METODO QUE ESCRIBE LA FECHA ACTUAL
